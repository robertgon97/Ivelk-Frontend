<template>
  <div>
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item :to="{ path: '/tienda/articulos/categoria' }">Lista de Categorías</el-breadcrumb-item>
    </el-breadcrumb>
    <br />
    <el-card class="mb-3">
        ...
        <form @submit.prevent="addStatus">
          ...
          <div class="row m-0 p-0 justify-content-center">
            <div class="col-md-6 mb-3">
              ...
              <input class="form-control" type="file" name="articulo_imagen" id="articulo_imagen" placeholder="Selecciona tu imagen" accept="image/*" @change="updateImage" />
            </div>
          </div>
          <div class="row m-0 p-0 justify-content-center">
            <div class="col-md-5 mb-3 pt-4 text-center">
              <el-button class="btn-primario mt-2" round @click="addStatus" v-loading.fullscreen.lock="uploading">Agregar Categoría</el-button>
            </div>
          </div>
        </form>
      </div>
    ...
    <div v-if="getAllCategoria && getAllCategoria == 'Loading'">
      <el-button type="text" v-loading.fullscreen.lock="true"></el-button>
    </div>
    <el-card class="mb-3" v-else-if="getAllCategoria && getAllCategoria.length">
      <div slot="header" class="clearfix">
      ...
      <div class="contenido">
        <el-table :data="getAllCategoria">
          <el-table-column fixed width="50" prop="articulo_tipo_id" label="#"></el-table-column>
          <el-table-column prop="articulo_tipo_nombre" label="Nombre"></el-table-column>
          <el-table-column prop="articulo_tipo_descripcion" label="Descripcion"></el-table-column>
          <el-table-column fixed label="Imagen" width="150">
            <template slot-scope="props">
              <div class="demo-basic--circle">
                <div class="block">
                  <el-avatar shape="square" size="medium" :src="props.row.articulo_imagen || '/noimage.png'"></el-avatar>
                </div>
              </div>
            </template>
          </el-table-column>
        ...
        <div>
          <div class="d-flex justify-content-center">
            <p class="m-0 p-0"><small>Mostrando {{getAllCategoria.length}} Registros</small></p>
          </div>
        </div>
      </div>
    </el-card>
...
<script>
  ...
  export default {
    metaInfo: {
      titleTemplate: '%s | Gestión de Categorías'
    },
    data () {
      return {
        uploading: false,
        ...
        article: {
          ...
          articulo_tipo_descripcion: null
        }
      }
    },
    methods: {
      addStatus () {
        ...
        this.formData.delete('articulo_tipo_nombre')
        this.formData.append('articulo_tipo_nombre', this.article.articulo_tipo_nombre)
        this.formData.delete('articulo_tipo_descripcion')
        this.formData.append('articulo_tipo_descripcion', this.article.articulo_tipo_descripcion)
        this.axios({
          method: `POST`,
          url: `/categoria`,
          data: this.formData
      ...
      updateImage (event) {
        ...
        this.formData.delete('articulo_imagen')
        if (files.length) {
          for (let image of files) {
            this.formData.append('articulo_imagen', image, image.name)
          }
        }
      },
      eliminar (marca) {
        this.$confirm('Deseas eliminar esta categoria?', 'Estás Seguro de esto?', {
        ...
        .then(() => {
          ...
          this.axios({
            ...
            url: `/categoria`,
            data: {
              articulo_tipo_id: marca.articulo_tipo_id,
              articulo_tipo_nombre: marca.articulo_tipo_nombre,
              articulo_tipo_descripcion: marca.articulo_tipo_descripcion,
              articulo_imagen: marca.articulo_imagen || 'aa'
            }
          })
          .then(() => {
            this.$notify({
              title: 'Eliinación Exitosa!',
              message: `La categoria ${marca.articulo_tipo_nombre} se eliminó correctamente!`,
              type: 'success'
            })
            this.$store.dispatch('startupEscencial')
            this.$store.dispatch('startupAdmin')
      ...
      print () {
        this.axios({
          method: `GET`,
          url: `/categoria?pdf=true`,
          responseType: 'blob'
        })
        .then((response) => {
          fileDownload(response.data, 'Categoría.pdf');
        })
      }
    },
    computed: {
